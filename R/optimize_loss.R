...
#' @export
optimize_loss <- function(X,H0,k,y,delta,theta,alpha,lambda,alpha=1,tol=0.01,maxit=5000,tol_H=1e-4,maxit_H=10000,step=1e-5,mu=.99){
  #initialize
  H <- H0
  loss <- 0
  eps <- 1
  ...
  while(eps > tol & it < maxit){
    ...
    # Update beta
    beta <- update_beta(H,y,delta,theta,lambda,alpha)
